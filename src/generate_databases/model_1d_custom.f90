...
!--------------------------------------------------------------------------------------------------
!
! 1D model profile using delimited text file (reused code from model_1d_cascadia)
!
! by Mark Valencia
!--------------------------------------------------------------------------------------------------

  subroutine model_1D_iasp91(xmesh,ymesh,zmesh,rho,vp,vs,qmu_atten,qkappa_atten)
...
! given a GLL point, returns super-imposed velocity model values
  ...
  ! TODO: read data from .dat or .csv file

  ! sets filename (default)
  filename = '1d_profile.dat'
  ...
  if (ier /= 0) then
    print *,'Error: could not open 1D profile file: ',trim(tomo_filename)
  ...
  end subroutine model_1D_iasp91
